	...
	: BaseRespWithData<WordSessionDetail>(code, message, wordSessionDetail)
{
	...
	public static StartStudyResp Fail(int code, string message) => new(code, message, null);
}
	...
	: BaseRespWithData<NextWordDetail>(code, message, nextWordDetail)
{
		...
		int studyingWordCount)
	{
		...
		return new GetNextWordResp(StatusCodes.Status200OK,
		SuccessMessages.Controller.StudySession.GetNextWordSuccess,
		new NextWordDetail
		{
			Options = options,
			Answer = answer,
	...
	public static GetNextWordResp Fail(int code, string message) => new(code, message, null);
}
