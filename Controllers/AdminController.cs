	...
	: ControllerBase
{
	...
	public async Task<IActionResult> GetAllWords([FromQuery] GetWordsByAdmin parameters)
	{
		try
		{
			var words = await repository.WordRepository.GetAllWordsAsync(parameters.Index,
			parameters.Size,
			parameters.Keyword);
	...
	public async Task<IActionResult> GetAllArticles([FromQuery] GetArticlesByAdmin parameters)
	{
		...
		{
			var articles =
				await repository.ArticleRepository.GetAllArticlesAsync(parameters.Index,
				parameters.Size,
				parameters.Keyword);
	...
	public async Task<IActionResult> GetAllRoles([FromQuery] GetRolesByAdmin parameters)
	{
		try
		{
			...
			var roleListResponse = new RoleListResp
			{
				List =
				[
					.. roles.Select(u => new RoleListDto
					{
						UserId = u.Id,
						RoleName = u.Role
					})
	...
	public async Task<IActionResult> RemoveRole([FromRoute] Guid userId)
	{
		try
		{
			...
			if (await repository.UserRepository.SaveAsync()) return Ok(ResponseFactory.NewSuccessBaseResponse(SuccessMessages.Controller.Admin.ChangeRoleSuccess));
