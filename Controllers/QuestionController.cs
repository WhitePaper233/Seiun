...
public class QuestionController(IRepositoryService repository) : ControllerBase
{
    ...
    public async Task<IActionResult> QuestionList([FromQuery] string questionType)
    {
        ...
        switch (questionType)
        {
            case "":
            case "all":
                questionList = (await repository.UserQuestionRepository.GetByUserId(userId.Value)).Select(u => u.QuestionId).ToList();
                break;
            case "fill-in-blank":
                questionList = (await repository.UserQuestionRepository.GetByUserIdAndQuestionType(userId.Value, QuestionType.FillInBlank)).Select(u => u.QuestionId).ToList();
                break;
            case "cloze-test":
                questionList = (await repository.UserQuestionRepository.GetByUserIdAndQuestionType(userId.Value, QuestionType.ClozeTest)).Select(u => u.QuestionId).ToList();
                break;
            default:
