...
public class CurrentStudySessionService(
    IServiceScopeFactory serviceScopeFactory,
    ILogger<CurrentStudySessionService> logger) : ICurrentStudySessionService
{
    ...
    {
        if (_currentStudySessions.TryAdd(sessionId, words)) return true;
    ...
    {
        if (_currentStudySessions.TryGetValue(sessionId, out var words)) return words;
        logger.LogWarning("Session {} does not exist", sessionId);
        return null;
    }
    ...
    public async Task ClearSessionAsync()
    {
        ...
        foreach (var session in _currentStudySessions)
        {
            ...
            if (hoursSpan.TotalHours <= 20) continue;
            clearingSessionIds.Add(session.Key);
        }

        foreach (var sessionId in clearingSessionIds) _currentStudySessions.Remove(sessionId);
    }
}